...
var AppController = /*#__PURE__*/function () {
  ...
  _createClass(AppController, [{
    ...
    value: function setCurrentGridSound(state, soundIndex, enabled) {
      var layer = this.helper.getCurrentLayer(state);
      var currentEnabled = layer.seq[state.currentGridIndex].sounds.includes(soundIndex);
      if (enabled == currentEnabled) return state;
      if (enabled) layer.seq[state.currentGridIndex].sounds.push(soundIndex);else layer.seq[state.currentGridIndex].sounds.splice(layer.seq[state.currentGridIndex].sounds.indexOf(soundIndex), 1);
      return state;
    }
  }, {
    ...
    value: function soundButtonClick(state, soundIndex) {
      if (state.recording) {
        ...
        if (!_layer.seq[gridIndex].sounds.includes(soundIndex)) _layer.seq[gridIndex].sounds.push(soundIndex);
        return state;
      }
    ...
    value: function chordButtonClick(state, octave) {
      ...
      if (state.recording) {
        ...
        if (!_layer2.seq[gridIndex].sounds.includes(octave * 3)) _layer2.seq[gridIndex].sounds.push(octave * 3);
        if (!_layer2.seq[gridIndex].sounds.includes(octave * 3 + 1)) _layer2.seq[gridIndex].sounds.push(octave * 3 + 1);
        if (!_layer2.seq[gridIndex].sounds.includes(octave * 3 + 2)) _layer2.seq[gridIndex].sounds.push(octave * 3 + 2);
        return state;
      }
